...
const SHIPPING = 5.00;
const PROMO_CODES = { 'HYDRO10': 0.10, 'AVENTURA': 0.15 };
let discount = 0;
...
function itemHTML(item) {
  return `
    <div class="cart-item" data-id="${item.id}" style="animation-delay:${Math.random() * 0.15}s">
...
function renderCart() {
  ...
  if (!container || !empty || !layout) return;
...
/* Item actions dynamic delegation */
...
if (applyPromoBtn) {
  applyPromoBtn.addEventListener('click', () => {
    ...
    if (!promoCodeInput || !msgEl) return;
...
/* Demo items if cart is empty on first visit */
if (Cart.count() === 0) {
  Cart.addItem({
    id: 'hf-wide-32',
    name: 'Hydro Flask Wide Mouth 32 oz',
    variant: 'Color: Azul Pacífico — 32 oz',
    price: 44.95,
